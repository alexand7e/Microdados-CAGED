...
import os
from utils_formatting import CagedUtils
from ftp_data_fetcher import SevenZipExtractor, FTPDownloader
from processed_table_caged import Processed_caged


class Microdata_caged:
    """
    A classe `Microdata_caged` é projetada para manipular microdados do CAGED (Cadastro Geral de Empregados e 
    Desempregados). Oferece funcionalidades para configurar o ambiente de trabalho, definir a competência 
    (mês e ano de referência) dos dados, especificar a unidade federativa de referência, e opcionalmente, 
    baixar os microdados.

    Atributos:
        file_path_micro (str, opcional): Caminho para o arquivo dos microdados. Se não fornecido e 
            `download_microdata` for True, os dados serão baixados para `local_directory`.
        local_directory (str): Diretório local onde os arquivos serão salvos ou lidos. 
            O padrão é o diretório de trabalho atual.
        mes_atual (int): Mês de referência para os dados. Por padrão, é o mês anterior ao mês atual.
        ano_atual (int): Ano de referência para os dados. Por padrão, é o ano do mês anterior ao mês atual.
        uf_ref (int): Código da Unidade Federativa (UF) de referência. O padrão é 22.
        download_microdata (bool): Se True, os microdados serão baixados automaticamente para `local_directory`.

    Métodos:
        __init__(self, file_path_micro=None, local_directory=os.getcwdb(), 
                 mes_atual=(datetime.now() - relativedelta(months=1)).month, 
                 ano_atual=(datetime.now() - relativedelta(months=1)).year, uf_ref=22, 
                 download_microdata=False): 
            Inicializa a instância da classe com os parâmetros fornecidos.

        baixar_microdados(self):
            Método para baixar os microdados. Deve ser implementado para realizar o download dos 
            dados para `file_path_micro` dentro de `local_directory`.

    Exemplo de Uso:
        # Criando uma instância da classe sem baixar os microdados
        microdados = Microdata_caged(
            file_path_micro="caminho/para/microdados.zip",
            local_directory="/caminho/para/diretorio/local",
            mes_atual=7,  # Julho
            ano_atual=2023,
            uf_ref=33,  # Rio de Janeiro
            download_microdata=False
        )

        # Criando uma instância da classe com download automático dos microdados
        microdados_com_download = Microdata_caged(
            local_directory="/caminho/para/diretorio/local",
            download_microdata=True
        )

    Nota:
        A classe depende de bibliotecas externas como pandas, numpy, os, e dateutil para manipulação 
        de datas e dados. As classes utilitárias `CagedUtils`, `SevenZipExtractor`, e `FTPDownloader` 
        são assumidas como parte da lógica de manipulação e download dos microdados. A funcionalidade de 
        download automático dos microdados oferece conveniência mas requer configuração adequada.
    """
    def __init__(self, 
                 file_path_micro: str = None,
                 local_directory: str = None,
                 mes_atual: int = (datetime.now() - relativedelta(months=2)).month,
                 ano_atual: int = (datetime.now() - relativedelta(months=2)).year,
                 uf_ref: int = 22,
        ...
        if file_path_micro is None and local_directory is None:
            ...
            self.file_path_micro = file_path_micro 
        ...
        self.data_base = f'{ano_atual}{str(mes_atual).zfill(2)}'
        self.dimensões = {}
    ...
    def baixar_microdados_e_dicionario(self):
        """
    ...
    def baixar_caged_processado(self):
        """A documentar"""
        try:
            caged_processed_downloader = Processed_caged(
                mes_referencia = self.L.classificar_mes(self.mes_atual),
    ...
    def importar_dicionario(self) -> pd.DataFrame:
        ...
        for pagename in self.categorias[:-1]:
            ...
            if pagename == "município":
                self.dimensões[pagename] = pd.read_excel(os.path.join(self.local_directory, "auxiliar", "dimensao_municipios.xlsx"), sheet_name=pagename)
            elif pagename == "Setores":
                lista = self.L.classificar_grupamento("", True) 
                self.dimensões[pagename] = pd.DataFrame({'Código': lista, 'Descrição': lista})
            elif pagename == "Escolaridade":
                lista = self.L.classificar_escolaridade("", True) 
                self.dimensões[pagename] = pd.DataFrame({'Código': lista, 'Descrição': lista})
            elif pagename == "competênciamov":
                lista = self.L.classificar_mes(None, True) 
                self.dimensões[pagename] = pd.DataFrame({'Código': [list(item.keys())[0] for item in lista], 
                ...
                self.dimensões[pagename] = pd.DataFrame({"Código": faixas, "Descrição":faixas})
            else:
                self.dimensões[pagename] = pd.read_excel(filename, sheet_name=pagename)
            
        return self.dimensões

    
    def importar_caged_tipo(self, ano, mes, tipo, data_inicial = None) -> pd.DataFrame:
        """
        ...
        caged['faixaetária'] = self.L.classificar_faixa_etaria(caged['idade'])
        caged['Período'] = caged["competênciamov"].apply(self.L.classificar_período)
        caged["valorsaláriofixo"] = caged["valorsaláriofixo"].apply(self.L.ajustar_coluna_decimal)
        caged["Setores"] = caged["seção"].apply(self.L.classificar_grupamento)
        caged["Escolaridade"] = caged["graudeinstrução"].apply(self.L.classificar_escolaridade)
    ...
    def importar_caged_mes_ano(self, get_categoria: bool = False):
        """
        ...
        for tipo in bases:
            for ano in self.L.ano_analisado(self.ano_atual):
                for mes in self.L.mes_analisado(self.mes_atual, self.ano_atual, self.ano_atual, self.mes_atual):
                    # Verifique se o tipo 'MOV' deve ser importado
                    if ano == int(self.ano_atual) and mes == int(self.mes_atual) and tipo == "MOV":
                        caged = self.importar_caged_tipo(ano, mes, tipo)
        ...
        if get_categoria:
            ...
            for categoria in self.categorias[:-1]:
                ...
                grupo = caged_with_base.groupby(['Período', categoria]).apply(lambda x: pd.Series({
                    'Salario_admissão': self.L.custom_mean(x.loc[x['saldomovimentação'] == 1, 'valorsaláriofixo']), 
                    'Salario_desligamento': self.L.custom_mean(x.loc[x['saldomovimentação'] == -1, 'valorsaláriofixo']),
    ...
    def importar_histórico_caged(self, ano_inicial: int = 2023, mes_inicial: int = 1):
        """
        ...
        caged_base = []  # Lista para armazenar os valores no loop
        result = {}
        data_base_histórico = self.L.classificar_período(f'{ano_inicial}{str(mes_inicial).zfill(2)}')

        bases = ["MOV", "EXC", "FOR"]

        for tipo in bases:
            for ano in self.L.ano_analisado(ano_inicial):
                for mes in self.L.mes_analisado(mes_inicial, ano_inicial, self.mes_atual):
                    caged = self.importar_caged_tipo(ano, mes, tipo, data_base_histórico)
                    caged_base.append(caged)
    ...
    def consolidar_caged(self, caged_agrupado: dict, caged_dimensoes: dict):
        """
        ...
        try:
            ...
            # Inserir histórico
            caged_formatado_completo[("Histórico - Estoque", "Histórico - Saldo", "Histórico - Variação")] = self.consolidar_historico()
    ...
    def analises_combinadas(self, df_microdados, caged_dimensoes, grupos = []):
        """
    ...
    def analisar_salarios_aprimorado(self, df_microdados, caged_dimensoes, dimensoes: list):
        """
        ...
        for dimensao in dimensoes:
            grupo = df_microdados.groupby(['Período', dimensao]).apply(lambda x: pd.Series({
                'Salario_total': self.L.custom_sum(x['valorsaláriofixo']),
                'Salario_medio': self.L.custom_mean(x['valorsaláriofixo']),
                'Contagem': x['valorsaláriofixo'].count(),
                'Salario_mediana': x['valorsaláriofixo'].median(),
                'Salario_admissao': self.L.custom_mean(x.loc[x['saldomovimentação'] == 1, 'valorsaláriofixo']),
                'Salario_desligamento': self.L.custom_mean(x.loc[x['saldomovimentação'] == -1, 'valorsaláriofixo'])
            }))
    ...
    def salvar_arquivos(self, file_path: str, dicionario_df: dict, filename: str):
        """
        ...
        """
        arquivo = pd.ExcelWriter(f'{file_path}/{filename}_{self.L.classificar_mes(self.mes_atual)}.xlsx')
    ...
    def get_data_final_dictionary(self, caged_agrupamentos: list):
        dataframes_dict = {}
        pages = self.get_categorias()
        for key in pages[:-1]:
